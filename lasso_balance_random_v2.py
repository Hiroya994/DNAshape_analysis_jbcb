...
def select_and_append(df, column_name='Gene_type', value='T', n=2):

    # 条件に合致する行と合致しない行を分けて抽出
    match_df = df[df[column_name] == value]
    not_match_df = df[df[column_name] != value]
    
    # 条件に合致する行からランダムにn行選択
    selected_rows = match_df.sample(n=min(n, len(match_df)), random_state=1)
    
    # 選択した行を条件に合致しない行に追加
    result_df = pd.concat([not_match_df, selected_rows], ignore_index=True)
...
data = select_and_append(data, n=21)
...
data = select_and_append(data, n=13)
...
data = select_and_append(data, n=17)
...
data = select_and_append(data, n=12)
